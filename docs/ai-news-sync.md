...
2. `npm run sync:ai-news` fetches the archive page, accepts only files named `ucsd-ai-newsletter-YYYY-MM-DD.md`, validates the required sections, removes executable markup, normalizes links, and writes new or changed editions to `content/newsletters/`.
