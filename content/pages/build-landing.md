...
<p><strong>API key required:</strong> An active TritonAI API key is a prerequisite for using the Harness. You can download the installer while access is pending, but setup cannot verify the connection or finish without an active key. Request Harness access before starting setup.</p>
<p class="build-harness-actions"><a class="btn btn-primary" href="/developer-apis/start.html">Request Harness access</a><a class="btn btn-default" href="https://github.com/dbalders/TritonAI-Installer/releases/latest">Download the TritonAI Harness installer <span class="glyphicon glyphicon-new-window" aria-hidden="true"></span></a><a class="build-harness-text-link" href="/skills/index.html">Browse compatible skills <span aria-hidden="true">→</span></a></p>
<h4>What to do after download</h4>
<ol class="harness-install-steps">
<li>On the GitHub release page, open <strong>Assets</strong>. Choose the file ending in <code>-arm64.dmg</code> for a Mac with Apple Silicon. For Windows x64, choose the file beginning with <code>TritonAI-Installer-Setup-</code> and ending in <code>-x64.exe</code>.</li>
<li>Open the downloaded file. On macOS, open the disk image and double-click <strong>TritonAI Installer</strong>. On Windows, run the Setup file. The Windows package may trigger Microsoft Defender SmartScreen because it is unsigned; confirm that the file came from the linked release and follow your department's software policy before continuing.</li>
<li>Enter your active TritonAI API key, select <strong>Check access &amp; install</strong>, and keep the Installer window open while setup verifies access and installs the managed components.</li>
<li>When <strong>Installation complete</strong> appears, select <strong>Open TritonAI Harness</strong>. You can open it later from Applications on macOS or from the desktop shortcut on Windows.</li>
</ol>
