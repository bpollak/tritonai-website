...
<div class="container">
<div class="hub-heading"><p class="home-kicker">The shared API path</p><h2 id="api-gateway-heading">Use the model route approved for your work</h2><p>Supported environments connect through the TritonAI LLM Gateway. In Harness, cloud and on-premises access use separate approved credentials. For campus administrative work, on-premises token use is not recharged. Cloud use is charged from the first token at the current market rate published in the Model Hub and requires a chartstring, named budget owner, and spend limit. Research projects charge both on-premises and cloud model use to a grant or approved research project chartstring. Other UC campuses may request Gateway access through an inter-campus recharge agreement and are recharged for both on-premises and cloud model use.</p></div>
